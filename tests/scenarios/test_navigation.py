...
def gpx_detour(tmp_path_factory):
    ...
    route = fetch_route(ORIGIN, DESTINATION)
    interpolated = interpolate_route(route, speed_kmh=60.0)
    detoured = apply_detour(interpolated)
...
class TestNavigation:
    ...
    def test_02_distance_update(
        ...
        with allure.step("잔여 거리 갱신 대기"):
            updated = nav_page.wait_for_distance_decrease(timeout=30)
            assert updated, "잔여 거리가 갱신되지 않음"

        with allure.step("갱신된 거리 확인"):
    ...
    def test_03_reroute_on_detour(
        self, appium_driver, nav_page, player_factory, gpx_detour
    ):
        """경로 이탈 시 재탐색 발생 확인 (핵심 시나리오)."""
        player = player_factory(gpx_detour, speed_multiplier=4.0)

        with allure.step("이탈 경로 GPX 재생 시작"):
            thread = player.play_async()

        with allure.step("재탐색 발생 대기 (최대 60초)"):
            rerouted = False
            for _ in range(60):
            ...
            allure.attach(
                str(rerouted), "재탐색 발생 여부", allure.attachment_type.TEXT
            )
            assert rerouted, "경로 이탈 후 재탐색이 발생하지 않음"

        with allure.step("재탐색 후 안내 문구 확인"):
            guidance = nav_page.get_current_guidance_text()
            allure.attach(guidance, "재탐색 후 안내 문구", allure.attachment_type.TEXT)
            assert guidance, "재탐색 후 안내 문구가 비어있음"
    ...
    def test_04_stop_maintains_guidance(
        self, appium_driver, nav_page, player_factory, gpx_stop
    ):
        ...
        with allure.step("주행 시작 후 안내 활성 확인"):
            time.sleep(5)
            assert nav_page.is_navigation_active(), "안내가 비활성화됨"
    ...
    def test_05_navigation_end(self, appium_driver, map_page, nav_page):
        ...
        with allure.step("현재 안내 상태 확인"):
            if not nav_page.is_navigation_active():
                pytest.skip("안내가 이미 종료된 상태")
